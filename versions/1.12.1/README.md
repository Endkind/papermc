...
```bash
docker run -it -d -p 25565:25565 --name endkind-papermc endkind/papermc:1.12.1
```
...
docker run -it -d -p 25565:25565 --name endkind-papermc -v endkind-papermc:/data -e MAX_RAM=3G --restart=always endkind/papermc:1.12.1
```
...
```bash
docker build -t endkind/papermc:1.12.1 .
```
