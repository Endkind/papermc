...
This Docker image provides PaperMC 1.21.11-rc2 Minecraft Server. You can easily run a Minecraft server using this image.

> [!CAUTION]
> This version is not bootable because `paper-1.21.11-rc2-{build}.jar` cannot be downloaded from the PaperMC API. This version is no longer actively updated but remains available for manual building.

## Quick start

```bash
docker run -it -d -p 25565:25565 --name endkind-papermc -e MINECRAFT_EULA=true endkind/papermc:1.21.11-rc2
```
...
docker run -it -d -p 25565:25565 --name endkind-papermc -v endkind-papermc:/data -e MAX_RAM=3G -e MINECRAFT_EULA=true --restart=always endkind/papermc:1.21.11-rc2
```
...
```bash
docker build -t endkind/papermc:1.21.11-rc2 .
```
