...
This Docker image provides PaperMC 1.8.8 Minecraft Server. You can easily run a Minecraft server using this image.

## Quick start

```bash
docker run -it -d -p 25565:25565 --name endkind-papermc endkind/papermc:1.8.8
```
...
docker run -it -d -p 25565:25565 --name endkind-papermc -v endkind-papermc:/data -e MAX_RAM=3G --restart=always endkind/papermc:1.8.8
```
...
```bash
docker build -t endkind/papermc:1.8.8 .
```
