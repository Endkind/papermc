...
def get_all_versions() -> Result[list[str], str]:
    try:
        ...
        return Err(
            f"Failed to fetch versions for project paper. Request error: {error}"
        )

    if response.status_code != 200:
        return Err(
            f"Failed to fetch versions for project paper. Status code: {response.status_code}"
        )
    ...
    try:
        ...
        return Err(
            f"Failed to fetch versions for project paper. Invalid JSON response: {error}"
        )

    versions_by_release = payload.get("versions")
    if not isinstance(versions_by_release, dict):
        return Err(
            f"Failed to fetch versions for project paper. Invalid response shape: missing or invalid 'versions' object"
        )
    ...
    for release, versions in versions_by_release.items():
        if not isinstance(versions, list) or not all(
            ...
            return Err(
                f"Failed to fetch versions for project paper. Invalid versions list for release '{release}'"
            )
